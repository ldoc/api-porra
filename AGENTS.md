...
- **Perfil invitado (`isGuest`) — reglas permanentes**: un usuario con `isGuest:true` se crea solo desde una invitación con `isGuest:true` (el admin la marca) y puede registrarse en cualquier fase. Nunca se le cierran pronósticos, plantilla ni cuadro final. Es **invisible para usuarios normales y anónimos**: todo endpoint que liste usuarios debe filtrar con `guestReadFilter(viewer)` (`api/guest.js`) y las lecturas individuales deben devolver 404 si el objetivo es guest y el espectador no es invitado ni admin. Nunca se incluye en estadísticas, clasificación, avatares tomados ni PDF que vean normales. En `GET /api/admin/progress` aparece con `isGuest:true`. **Toda feature nueva que agregue usuarios o calcule puntos debe contemplar este perfil.**
